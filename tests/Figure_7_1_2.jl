...
#Algorithms options
timelim=30.0  #for results in the paper use 90.0
maxiter=200000
tol=1e-6
...
D_l1=Dict(); D_l2=Dict(); D_l21=Dict(); D_kl=Dict();
...
for i in 1:n_v
    ...
    for j in 1:rep
        ...
        #Compute wL1-NMF 
        W_l1, H_l1 = sCD(X, r, lambda = lambda, W0=W, H0=H, timelim=timelim, maxiter=maxiter,tol=tol)
        ...
        if j==1
            D_l1[i]=T_l1; D_l2[i]=T_l2; D_l21[i]=T_l21; D_kl[i]=T_KL
...
ss=9;
ind=randperm(samp)[1:ss]
for i in 1:n_v
    for k in ind
        D_l1[i][:,k]=vec(reverse(reshape(D_l1[i][:,k],28,28)', dims=1))
        D_l2[i][:,k]=vec(reverse(reshape(D_l2[i][:,k],28,28)', dims=1))
        D_l21[i][:,k]=vec(reverse(reshape(D_l21[i][:,k],28,28)', dims=1))
        D_kl[i][:,k]=vec(reverse(reshape(D_kl[i][:,k],28,28)', dims=1))
    end
end
for i in 1:n_v 
    L1NMF.affichage(D_l1[i][:,ind]', 28, 28, "L1_noise$(noise_vec[i]).png",nbimgperrow=3)
    L1NMF.affichage(D_l2[i][:,ind]', 28, 28, "L2_noise$(noise_vec[i]).png",nbimgperrow=3)
    L1NMF.affichage(D_l21[i][:,ind]', 28, 28, "L21_noise$(noise_vec[i]).png",nbimgperrow=3)
    L1NMF.affichage(D_kl[i][:,ind]', 28, 28, "KL_noise$(noise_vec[i]).png",nbimgperrow=3)
end
##
#Uncomment if you need to store the result (adjust the path)
