...
# wL1-NMF model
lambda_vec=[0.08,1.0] #define the parameters of the model
...
for j in 1:p
    ...
    W2, H2, times2,errors2,it2 = sCD(X, r, lambda = lambda, W0=W0, H0=H0, maxiter=maxiter, timelim=timelim,tol=tol)
    push!(cpu_it,times2[end]/it2)
...
#Display the table with the number of non reliable topics 
v = vcat("",fill("wL1-NF", p), "FroNMF")
table=vcat(lambda_vec',round.(Int,w_per_topic)',count_null_l1')
table=hcat(["lambda";"words x topic";"null topics"],table)
pretty_table(table; column_labels = v, alignment = :c)
