...
maxiter=10  #maxiter=30 used in the paper
...
sparse_param=[0.25,0.50]; ns=length(sparse_param);
...
for k=1:nd
    ...
    for j=1:ns
        ...
        for i=1:rep
            ...
            #Run standard BCD
            W4, H4, times4,errors4, it4 = CD(X, r, lambda = lambda, W0=W0, H0=H0, maxiter=maxiter, timelim=timelim,  tol=tol)
...
##
header = ["m×n", "sparsity", "sCD", "CD", "σ", "gain"]
table = permutedims(hcat(rows...))
pretty_table(table; column_labels = header, alignment = :c)
##
##
